...
from verl.models.fin_prm import Qwen3ForProcessRewardModel
from verl.models.fin_config import Qwen3PRMConfig
...
class RewardModelWorker(Worker):
    ...
    def _build_model(self, config):
        ...
        model_config = Qwen3PRMConfig.from_pretrained(local_path, trust_remote_code=trust_remote_code)
        ...
        with init_context(), warnings.catch_warnings():
            ...
            #     trust_remote_code=trust_remote_code,
            # )
            reward_module = Qwen3ForProcessRewardModel.from_pretrained(
                pretrained_model_name_or_path=local_path,
                # config=model_config,
                torch_dtype=torch.bfloat16,
                attn_implementation="flash_attention_2",
    ...
    def _expand_to_token_level(self, data: DataProto, scores: torch.Tensor):
        ...
        # print(f'token_level_scores.shape: {token_level_scores.shape}\neos_mask_idx.shape: {eos_mask_idx.shape}\nscores.shape: {scores.shape}')
    ...
    def _switch_chat_template(self, data: DataProto):
        ...
        for i in range(data.batch.batch_size[0]):
            ...
            prompt = src_tokenizer.decode(data.batch["prompts"][i].to("cpu"),skip_special_tokens=True)
            response = src_tokenizer.decode(data.batch["responses"][i].to("cpu"),skip_special_tokens=True)
    ...
    def compute_rm_score(self, data: DataProto):
        ...
        ground_truths_batch = data.non_tensor_batch['reward_model']
        ground_truths = [p['ground_truth'] for p in ground_truths_batch]
        
        answers_batch = data.batch['responses']
        answers = [self.tokenizer.decode(ids, skip_special_tokens=True) for ids in answers_batch]
        
        rulebased_res = [math_dapo.compute_score(answer,ground_truth)['score'] for answer,ground_truth in zip(answers,ground_truths)]
        ...
        with self.ulysses_sharding_manager:
            ...
            for micro_batch in micro_batches:
                ...
                step_sep_id_0 = self.tokenizer.encode("<extra_0>")[0]
                step_sep_id_1 = self.tokenizer.encode("<extra_1>")[0]
                token_masks = (micro_batch['input_ids'] == step_sep_id_0) | (micro_batch['input_ids'] == step_sep_id_1)
                step_reward = make_step_rewards(rm_score[0], token_masks)
                # output.append(rm_score)
                output.append(step_reward[0][0])
            scores = torch.tensor([o1 for o1 in output])
